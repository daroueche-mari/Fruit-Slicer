...
IMAGES_LIST = {
    ...
    "spinner": "shuriken.png", 'eclair': 'eclair.png'
}
...
for name, filename in IMAGES_LIST.items():
    try:
        ...
        c = ICE_BLUE if name == "ice block" else (ORANGE_ELECTR if name == "eclair" else GREEN)
...
class GameObject:
    def __init__(self, speed_mult=1.0):
        ...
        if is_overcharged:
            self.type = random.choice([k for k in IMG_DATA.keys() if k not in ["bombe", "spinner", "ice block", "eclair"]])
        else:
            rand = random.random()
            if rand < 0.05: self.type = "ice block"
            elif rand < 0.10: self.type = "eclair"
            elif rand < 0.18: self.type = "spinner" 
            elif rand < 0.28: self.type = "bombe"   
            else: self.type = random.choice([k for k in IMG_DATA.keys() if k not in ["spinner", "bombe", "ice block", "eclair"]])
        ...
        # --- PROPULSION INITIALE (Réglage de la hauteur de saut) ---
        # On augmente un peu ici pour compenser le nombre élevé de fruits
        self.vy, self.vx = random.uniform(-16, -21) * speed_mult, random.uniform(-1.5, 1.5)
    ...
    def draw(self, surf):
        ...
        color = ICE_BLUE if self.type == "ice block" else (ORANGE_ELECTR if self.type == "eclair" else YELLOW)
...
while running:
    ...
    if game_mode in ["PLAY", "PAUSE"]:
        if game_mode == "PLAY":
            ...
            for s in slices[:]: s.update(); (slices.remove(s) if s.y > HEIGHT + 100 else None)
            for obj in active_objects[:]:
                obj.move(slow=is_iced)
                if obj.y > HEIGHT + 100:
                    if not is_iced and obj.type not in ["bombe", "ice block", "eclair"] and current_sub_mode == "CLASSIC": vies -= 1
        ...
        for s in slices: s.draw(game_surf)
        for l in lightnings: l.draw(game_surf)
        for obj in active_objects: obj.draw(game_surf)
        for sl in slashes: pygame.draw.line(game_surf, WHITE, sl["start"], sl["end"], 15)

        off = [random.randint(-shake_amount, shake_amount), random.randint(-shake_amount, shake_amount)] if shake_amount > 0 else [0,0]
        if game_mode == "PLAY": shake_amount = max(0, shake_amount - 1)
