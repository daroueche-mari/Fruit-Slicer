...
menu = pygame.image.load(os.path.join(IMG_DIR, "menu.png")).convert()
menu = pygame.transform.smoothscale(menu, (WIDTH, HEIGHT))
title_original = pygame.image.load(os.path.join(IMG_DIR, "title.png")).convert_alpha()
# Titre: on le redimensionne UNE fois pour le menu (évite de le re-scaler à chaque frame)
TITLE_MENU_W = 400
title_ratio = TITLE_MENU_W / title_original.get_width()
TITLE_MENU_H = int(title_original.get_height() * title_ratio)
title_menu = pygame.transform.smoothscale(title_original, (TITLE_MENU_W, TITLE_MENU_H))
fondgame = pygame.image.load(os.path.join(IMG_DIR, "fondgame.png")).convert_alpha()
fondgame = pygame.transform.smoothscale(fondgame, (WIDTH, HEIGHT))
...
sounds = {
    "start": pygame.mixer.Sound(os.path.join(SOUND_DIR, "main_start.mp3")),
    "slash": pygame.mixer.Sound(os.path.join(SOUND_DIR, "slash_impact.mp3")),
    "ice_bonus": pygame.mixer.Sound(os.path.join(SOUND_DIR, "ice_bonus.mp3")),
    "ambiance": pygame.mixer.Sound(os.path.join(SOUND_DIR, "ambiance.mp3")),
    "ambiance_game": pygame.mixer.Sound(os.path.join(SOUND_DIR, "ambiance_game.mp3")),
    "strike": pygame.mixer.Sound(os.path.join(SOUND_DIR, "strike.mp3")),
    "explode": pygame.mixer.Sound(os.path.join(SOUND_DIR, "Explode.mp3")),
    "projection_fruits": pygame.mixer.Sound(os.path.join(SOUND_DIR, "projection_fruits.mp3")),
    "projection_icecube": pygame.mixer.Sound(os.path.join(SOUND_DIR, "projection_iceblock.mp3")),
    "projection_spinner": pygame.mixer.Sound(os.path.join(SOUND_DIR, "projection_spinner.mp3")),
    "projection_bomb": pygame.mixer.Sound(os.path.join(SOUND_DIR, "projection_bomb.wav")),
    "light_bonus": pygame.mixer.Sound(os.path.join(SOUND_DIR, "lightning.wav")),
    "slash_combo": pygame.mixer.Sound(os.path.join(SOUND_DIR, "slash_combo.mp3")),
    "spiner_bonus": pygame.mixer.Sound(os.path.join(SOUND_DIR, "spiner_bonus.mp3"))
}
ambiance_channel = pygame.mixer.Channel(0)
ambiancegame_channel = pygame.mixer.Channel(1)
slash_combo = pygame.mixer.Channel(2)
ice_bonus = pygame.mixer.Channel(3)
projection_fruits = pygame.mixer.Channel(4)
projection_icecube = pygame.mixer.Channel(5)
...
class GameObject:
    ...
    def __init__(self, speed_mult=1.0):
        ...
        if is_overcharged:
            ...
            if rand < 0.05: 
                self.type = "ice_block"
                sounds["projection_icecube"].play()
            elif rand < 0.10: 
                self.type = "lightning"
                sounds["projection_fruits"].play()
            elif rand < 0.18: 
                self.type = "shuriken"
                sounds["projection_spinner"].play()
            elif rand < 0.28: 
                self.type = "bomb"
                sounds["projection_bomb"].play()
            else:
...
ambiance_channel.play(sounds["ambiance"])
while running:
            ...
            if game_state == "MENU":
                if WIDTH // 2 - 150 < mouse_pos[0] < WIDTH // 2 + 150:
                    if 280 < mouse_pos[1] < 340: 
                        ...
                        ambiancegame_channel.play(sounds["ambiance_game"])
                        sounds["start"].play()
                        reset_game("CLASSIC")
                ...
                elif pygame.Rect(WIDTH // 2 - 150, 380, 300, 60).collidepoint(mouse_pos): 
                    game_state = "MENU"
                    ambiance_channel.stop()
                    ambiancegame_channel.stop()
                    ambiance_channel.play(sounds["ambiance"])
                ...
                elif pygame.Rect(WIDTH // 2 - 150, 410, 300, 60).collidepoint(mouse_pos): 
                    game_state = "MENU"
                    ambiance_channel.stop()
                    ambiancegame_channel.stop()
                    ambiance_channel.play(sounds["ambiance"])
            ...
            if game_state == "PLAY":
                ...
                if event.key == pygame.K_ESCAPE: game_state = "PAUSE"
                    ...
                    if event.key == pygame.K_SPACE and is_overcharged and special_gauge >= MAX_GAUGE:
                        ...
                        special_gauge = 0
                        shake_intensity = 25
                        slash_combo.play(sounds["slash_combo"])
                        ...
                        if hit:
                            ...
                            if obj.hp <= 0:
                                if obj.type == "bomb":
                                    ...
                                    if is_overcharged: 
                                        special_gauge = min(MAX_GAUGE, special_gauge + 10)
    ...
    if game_state in ["PLAY", "PAUSE"]:
        ...
        # --- Rendu Visuel ---
        game_surface.blit(fondgame, (0, 0))
